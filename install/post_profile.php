...
$stmt = $dbh->query(
    "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'sys_users'"
);

if ((int) $stmt->fetchColumn() === 0) {
    exit('Database schema was not imported completely. Please run the installer again from step 3.');
}
...
if ($stmt->rowCount() === 1) {
    ...
    $stmt = $dbh->query("SELECT id FROM sys_users ORDER BY id ASC LIMIT 1");

    if ((int) $stmt->fetchColumn() === 0) {
        exit('No admin user was found after database import.');
    }
}
