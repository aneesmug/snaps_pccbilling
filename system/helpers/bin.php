...
foreach ($result as $value) {
    // ZATCA FIX: Handle NULL values by converting to empty strings
    // This prevents 409 recovery from failing due to NULL in $config
    $configValue = $value['value'];
    $config[$value['setting']] = ($configValue === null ? '' : $configValue);
}
...
function authenticate_admin()
{
    ...
    if ($user) {
        $user->preferences = UserPreference::getAllPreferences($user->id);
        global $ui;
        $ui->assign('user', $user);
        return $user;
    }
...
function ui()
{
    global $config, $_L, $file_build, $resourceURL, $clx_language_code, $req;
    ...
    $ui->assign('current_route', isset($req) ? trim($req, '/') : '');
    ...
    $ui->registerPlugin('modifier', 'str_ends_with', 'str_ends_with');
    $ui->registerPlugin('modifier', 'app_csrf_token', 'app_csrf_token');
    $ui->registerPlugin('modifier', 'app_csrf_field', 'app_csrf_field');
    $ui->registerPlugin('modifier', 'strtoupper', 'strtoupper');
    $ui->registerPlugin('modifier', 'appHumanReadableDuration', 'appHumanReadableDuration');
...
function sp_get_contact_image($contact, $background_color = null)
{
    ...
    if ($contact->img == '') {
        ...
        {
            $img = '<span class="clx-avatar" style="background-color: '.$background_color.'; color: #fff;">' . $two_l . '</span>';
        }
        else
        {
...
function appOpenAIClient($config)
{
    $api_key = $config['openai_api_key'] ?? null;
    ...
    if(!empty($config['openai_api_base_url']))
    {
        $client = $client->withBaseUri($config['openai_api_base_url']);
    }
...
function app_validate_csrf_token($token) {
    ...
    // Check if token exists in session and matches the submitted token
