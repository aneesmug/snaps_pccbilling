...
switch ($action) {
    ...
    case 'invoices':
        ...
        if ($d) {
            ...
            $dues_summary = CustomerDue::getSummaryForCustomer($d->id);
            ...
            view('ajax-contact-invoices', [
                ...
                'total_unpaid_amount' =>
                    ...
                    $dues_summary['total_unpaid_amount'],
    ...
    case 'print-statement':
        Event::trigger('contacts/print-statement/');
        ...
        require APP_SYSTEM_PATH . '/lib/invoices/customer-statement-print.php';

        break;

    case 'dues':
        Event::trigger('contacts/dues/');
        ...
        if ($d) {
            ...
            view('ajax-contact-dues', [
                'cid' => $cid,
                'dues' => $dues_summary['dues'],
                'total_due_amount' => $dues_summary['total_due_amount'],
                'total_paid_amount' => $dues_summary['total_paid_amount'],
                'total_unpaid_amount' => $dues_summary['total_unpaid_amount'],
            ]);
        }
    ...
    case 'due-add-post':
        Event::trigger('contacts/due-add-post/');
        ...
        if (!is_numeric($amount) || (float) $amount <= 0) {
            r2(U . 'contacts/view/' . $cid . '/dues/', 'e', $_L['amount_error']);
            break;
        }
        ...
        $due->customer_id = $cid;
        $due->amount = $amount;
        $due->paid_amount = '0.00';
        $due->description = $description;
        $due->date = $date;
        $due->status = 'Unpaid';
        $due->created_by = $user->id;
        $due->save();

        $due->voucher_no = 'DV-' . str_pad($due->id, 5, '0', STR_PAD_LEFT);
        $due->save();

        _log(
            $_L['Amount'] . ' ' . $amount . ' ' . $_L['Due Amount'] . ' ' .
                $due->voucher_no . ' ' . 'Added by [' . $user->fullname . ']' .
    ...
    case 'due-payment-post':
        Event::trigger('contacts/due-payment-post/');

        $due_id = _post('due_id');
        $amount = _post('amount');

        $due = CustomerDue::find($due_id);
        ...
        $customer = ORM::for_table('crm_accounts')->find($due->customer_id);
        ...
        if (!is_numeric($amount) || (float) $amount <= 0) {
            r2(U . 'contacts/view/' . $due->customer_id . '/dues/', 'e', $_L['amount_error']);
            break;
        }

        $balance = (float) $due->amount - (float) $due->paid_amount;
        $amount = min((float) $amount, $balance);

        $due->paid_amount = (float) $due->paid_amount + $amount;

        if ((float) $due->paid_amount >= (float) $due->amount) {
            $due->status = 'Paid';
        } elseif ((float) $due->paid_amount > 0) {
        ...
        _log(
            $_L['Amount'] . ' ' . $amount . ' ' . $_L['Add Payment'] . ' ' .
                $due->voucher_no . ' Recorded by [' . $user->fullname . ']' .
        ...
        r2(
            U . 'contacts/view/' . $due->customer_id . '/dues/',
            's',
            $_L['added_successful']
        );
    ...
    case 'due-delete':
        Event::trigger('contacts/due-delete/');
        ...
        if ((float) $due->paid_amount != 0.0) {
            r2(U . 'contacts/view/' . $cid . '/dues/', 'e', $_L['Cannot Delete']);
            break;
        }
    ...
    case 'print-voucher':
        Event::trigger('contacts/print-voucher/');
        ...
        $d = Contact::find($due->customer_id);
        ...
        require APP_SYSTEM_PATH . '/lib/invoices/customer-due-voucher-print.php';
    ...
    case 'add-post':
        ...
        $company_id = _post('cid'); // '__new__' or existing company ID
        ...
        if ($msg == '') {
            ...
            if ($buyer_type === 'company') {
                $d->tax_number    = $vat_number;   // VAT 15-digit
                $d->entity_number = $crn_number;   // Unified 700#
            } elseif ($buyer_type === 'individual') {
                $d->entity_number = $id_iqama;     // ID/Iqama
            }
    ...
    case 'map-data':
        ...
        $contacts = Contact::query()->where(function ($query) {
            $query
                ->where('type', 'Customer')
                ->orWhere('type', 'Customer,Supplier');
        ...
        {
            $contacts = $contacts->where('gid', $selected_group_id);
        }
        ...
        foreach ($contacts as $contact) {
            $color = '#3979FF';
            if(!empty($groups[$contact->gid]) && !empty($groups[$contact->gid]->color))
            {
                $color = $groups[$contact->gid]->color;
            }
            $contacts_data[] = [
                'id' => $contact->id,
                ...
                'lat' => $contact->lat,
                'lon' => $contact->lon,
                'image' => sp_get_contact_image($contact, $color),
            ];
        }

        api_response($contacts_data);

        break;

    case 'map-update-center':

        $json_data = getJsonParams();
        $latitude = $json_data->get('latitude');
        $longitude = $json_data->get('longitude');

        if(!empty($latitude) && !empty($longitude))
        {
            updateOption('contacts_map_center_latitude', $latitude, true);
            updateOption('contacts_map_center_longitude', $longitude, true);
        }

        break;

    case 'map-update-zoom':

        $json_data = getJsonParams();

        updateOption('contacts_map_zoom', $json_data->get('zoom'), true);
...
";
    ...
    case 'json-single-contact':
        ...
        if ($contact) {
            ...
            $buyer_type = strtolower(trim((string) $contact->buyer_type));
        }
