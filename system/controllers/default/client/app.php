...
switch ($action) {
    ...
    case 'iview':
        ...
        if ($d) {
            ...
            $ui->assign('invoice_type_label', $_effective_invoice_type === 'standard' ? 'Standard Tax Invoice' : 'Simplified Tax Invoice');
            ...
            $show_zatca_qr = !empty($config['invoice_show_qr_code']) && (string) $config['invoice_show_qr_code'] === '1' && !empty($qr_code);

            $seller_name = isset($zatca_data['seller_name']) && trim((string) $zatca_data['seller_name']) !== ''
                ? $zatca_data['seller_name']
                : $config['CompanyName'];
            ...
            $invoice_ref = isset($zatca_data['invoice_number']) && trim((string) $zatca_data['invoice_number']) !== ''
                ? $zatca_data['invoice_number']
                : $d['invoicenum'] . ($d['cn'] != '' ? $d['cn'] : $d['id']);
            ...
            if (is_callable(['Finance', 'amount_to_words_bilingual'])) {
                $amount_words = call_user_func(['Finance', 'amount_to_words_bilingual'], (float) $d['total']);
            } else {
                $amount_words = [
                    'en' => Finance::convert_number_to_words((float) $d['total']),
                    'ar' => '',
                ];
            }
            ...
            {
                $contract = Contract::find($d->contract_id);
            }
            ...
            view('client-iview', [
                ...
                'zatca_qr_phase_label' => $zatca_qr_phase_label,
                'seller_name' => $seller_name,
                'seller_vat' => $seller_vat,
                'seller_crn' => $seller_crn,
...
//        }
                ...
                case 'stripe_post':
                    ...
                    if ($p) {
                        ...
                        try {
                            ...
                            // Support both legacy and modern Stripe exception classes.
                            if (method_exists($e, 'getJsonBody')) {
                                $body = call_user_func([$e, 'getJsonBody']);
                                $err = isset($body['error']) && is_array($body['error']) ? $body['error'] : [];

                                if (method_exists($e, 'getHttpStatus')) {
                                    print 'Status is:' . call_user_func([$e, 'getHttpStatus']) . "\n";
                                }
    ...
    case 'btpay_submitted':
        ...
        if ($p) {
            ...
            $btConfigClass = class_exists('Braintree_Configuration')
                ? 'Braintree_Configuration'
                : (class_exists('Braintree\\Configuration') ? 'Braintree\\Configuration' : '');

            $btTransactionClass = class_exists('Braintree_Transaction')
                ? 'Braintree_Transaction'
                : (class_exists('Braintree\\Transaction') ? 'Braintree\\Transaction' : '');

            if ($btConfigClass === '' || $btTransactionClass === '') {
                r2(U . 'client/iview/' . $d->id . '/token_' . $token . '/', 'e', 'Braintree SDK is not available.');
                break;
            }

            call_user_func([$btConfigClass, 'environment'], $environment);
            call_user_func([$btConfigClass, 'merchantId'], $merchantId);
            call_user_func([$btConfigClass, 'publicKey'], $publicKey);
            call_user_func([$btConfigClass, 'privateKey'], $privateKey);
            ...
            if ($nonce) {
                ...
                if ($d) {
                    ...
                    if ($token != $vtoken) {
                        ...
                        $result = call_user_func([$btTransactionClass, 'sale'], [
                            'amount' => $amount,
                            'orderId' => $id,
...
vMax: \'9999999999999999.00\',
    ...
    case 'profile-picture-upload':
        ...
        if ($uploader->uploadFile('file')) {
            ...
            if (is_object($img) && method_exists($img, 'crop')) {
                call_user_func([$img, 'crop'], 300, 300);
            }
 ...
 ';
    ...
    case 'form_submitted':
        ...
        if ($id !== '' && $id !== '0') {
            ...
            $success_message = $success_message_original;
            $smartyClass = class_exists('Smarty')
                ? 'Smarty'
        ...
        .paper {
            ...
            case 'create_post':
                ...
                if ($config['recaptcha'] == 1 && !$_SESSION['recaptcha_verified']) {
                    ...
                    if (class_exists('Ib_Recaptcha') && is_callable(['Ib_Recaptcha', 'isValid'])) {
                        $recaptchaValid = (bool) call_user_func(
                            ['Ib_Recaptcha', 'isValid'],
                            $config['recaptcha_secretkey']
                        );
                    } elseif (function_exists('sp_verify_recaptcha')) {
                        $recaptchaResult = sp_verify_recaptcha(
                            $config['recaptcha_secretkey'],
                            _post('token')
                        );
                        $recaptchaValid = !empty($recaptchaResult) && !empty($recaptchaResult->success);
                    }
    ...
    case 'payment-stripe':
        ...
        if ($invoice && $payment_gateway) {
            ...
            $amount = $invoice_due_amount;
            $amount_submitted = $invoice_due_amount;

            if($invoice->allow_partial_payment)
            {
                $amount_submitted = _post('amount');
            ...
            if ($charge->status !== null && $charge->status == 'succeeded') {
                ...
                if($invoice->allow_partial_payment)
                {
                    if($amount_submitted > $invoice_due_amount)
                    {
