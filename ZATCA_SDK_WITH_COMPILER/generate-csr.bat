...
echo.
echo [2/6] Organization Unit (Branch Name)
...
echo csr.organization.unit.name=!UNIT!>> "!CONFIG_FILE!"
...
call "%~dp0Apps\fatoora.bat" -csr -pem -csrConfig "Data\Input\snaps-csr-config.properties" -privateKey "Data\Certificates\snaps-private-key.pem" -generatedCsr "Data\Certificates\snaps-request.csr"
set "SDK_EXIT=%ERRORLEVEL%"
echo   SDK exit code: !SDK_EXIT!

echo.
if exist "%~dp0Data\Certificates\snaps-private-key.pem" (
    if exist "%~dp0Data\Certificates\snaps-request.csr" (
        echo ============================================================
        echo   SUCCESS - Files generated:
        echo.
        echo   Private Key : Data\Certificates\snaps-private-key.pem
        echo   CSR File    : Data\Certificates\snaps-request.csr
        echo.
        echo   NEXT STEPS:
        echo   1. Submit snaps-request.csr to ZATCA Fatoora portal
        echo   2. Paste snaps-private-key.pem content into your
