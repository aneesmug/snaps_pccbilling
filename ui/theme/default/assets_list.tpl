            ...
            <div class="card border m-lg-0 mb-3">
                ...
                <div class="card-body mb-3">
                    {if $selected_category}
                        <a href="javascript:;" id="btnNewCategory" class="btn btn-primary mb-3">{$_L['New category']}</a>
                        <a href="javascript:;" id="btnEditCategory" class="btn btn-primary mb-3">{$_L['Edit']}</a>
                        <a href="javascript:;" onclick="confirmThenGoToUrl(event,'delete/asset-category/{$selected_category->id}');" class="btn btn-danger mb-3">{$_L['Delete']}</a>
                    {else}
            ...
            $('#btnNewCategory').click(function (e) {
                ...
                (async () => {
                    const { value: category } = await Swal.fire({
                        ...
                        input: 'text',
                        inputPlaceholder: '{__('Enter category name')}',
                        showCancelButton: true,
                    });

                    if (category){
                        $.post( "{$_url}assets/category-post", { category: category } ).done(function() {
                            location.reload();
                        });
                    }
            ...
            $('#btnEditCategory').on('click',function (event) {
                ...
                (async () => {
                    const { value: category } = await Swal.fire({
                        ...
                        input: 'text',
                        inputValue: '{$selected_category->name}',
                        showCancelButton: true,
                    });

                    if (category){
                        $.post( "{$_url}assets/category-post", { category: category, id: {$selected_category->id} }).done(function() {
                            location.reload();
                        });
                    }
