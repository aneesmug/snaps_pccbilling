...
{block name="content"}
<!-- Accounting System - Vendor Statement -->
<div class="row"><div class="col-md-12"><div class="card"><div class="card-header"><h2 class="card-title"><i class="fal fa-file-text"></i> {$_title|default:"Accounting"}</h2></div><div class="card-body">
    <div class="row mb-4">
        ...
        <div class="col-md-4 text-right">
            ...
            <a href="{$_url}accounts/ap-dashboard/dashboard" class="btn btn-secondary">
    ...
    <div class="row mb-4">
        <div class="col-md-6">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title text-muted">{$_L['Vendor Name']}</h6>
                    <p class="mb-0"><strong>{$vendor->name}</strong></p>
                    <p class="text-muted"><small>{$vendor->address}</small></p>
                </div>
            </div>
        </div>
        <div class="col-md-6">
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <h6 class="card-title text-muted">{$_L['Total Payable']}</h6>
                            <h4 class="text-danger">{$summary->total_payable|number_format:2}</h4>
                        </div>
                        <div class="col-md-6">
                            <h6 class="card-title text-muted">{$_L['Current Due']}</h6>
                            <h4 class="text-danger">{$summary->current|number_format:2}</h4>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <table class="table table-sm print-table mb-0 datatable">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th class="text-right">{$_L['Bill Amount']}</th>
                    ...
                    <tbody>
                        ...
                        {foreach $transactions as $txn}
                            {assign var="balance" value=$balance + $txn['bill_amount'] - $txn['payment']}
                            <tr>
                                <td>{$txn['date']}</td>
                                <td>
                                    {if $txn['type'] == 'bill'}
                                        <span class="badge badge-warning">{$_L['Bill']}</span>
                                    {else}
                                        <span class="badge badge-info">{$_L['Payment']}</span>
                                    {/if}
                                    {$txn['reference']}
                                </td>
                                <td class="text-right">{if $txn['bill_amount']}{$txn['bill_amount']|number_format:2}{else}â€”{/if}</td>
