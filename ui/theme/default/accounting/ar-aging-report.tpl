...
{block name="content"}
<!-- Accounting System - AR Aging Report -->
{assign var="total_current" value=0}
{assign var="total_30" value=0}
{assign var="total_60" value=0}
{assign var="total_90" value=0}
{foreach $aging|default:[] as $customer_id => $bucket}
    {assign var="total_current" value=$total_current + ($bucket.current|default:0)}
    {assign var="total_30" value=$total_30 + ($bucket['30']|default:0)}
    {assign var="total_60" value=$total_60 + ($bucket['60']|default:0)}
    {assign var="total_90" value=$total_90 + ($bucket['90']|default:0)}
{/foreach}
...
<div class="row"><div class="col-md-12"><div class="card"><div class="card-header"><h2 class="card-title"><i class="fal fa-file-text"></i> {$_title|default:"Accounting"}</h2></div><div class="card-body">
    <div class="row mb-4">
        <div class="col-md-8">
            <h2 class="mb-0">
                ...
                {$_L['Accounts Receivable Aging Report']}
            </h2>
        </div>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="get" class="form-inline">
                <input type="hidden" name="ng" value="accounts/ar-dashboard/aging">
                <div class="form-group mr-3">
                    <label class="mr-2">{$_L['As of Date']}:</label>
                    <input type="text"  name="as_of_date" class="form-control" value="{$as_of_date|default:$smarty.now|date_format:'%Y-%m-%d'}" datepicker data-date-format="yyyy-mm-dd" data-auto-close="true">
                </div>
    ...
    <div class="card">
        <div class="card-header bg-light">
            <h5 class="mb-0">{$_L['AR Aging Detail']}</h2>
        </div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-sm table-striped table-bordered print-table mb-0 datatable" id="aging_table">
                    ...
                    <tbody>
                        {foreach $aging|default:[] as $customer_id => $customer}
                            <tr>
                                <td><strong>{$customer.customer_name|default:'Unknown'}</strong></td>
                                <td class="text-right">{($customer.current|default:0)|number_format:2}</td>
                                <td class="text-right">{($customer['30']|default:0)|number_format:2}</td>
                                <td class="text-right">{($customer['60']|default:0)|number_format:2}</td>
                                <td class="text-right text-danger">{($customer['90']|default:0)|number_format:2}</td>
                                <td class="text-right font-weight-bold">
                                    {(($customer.current|default:0) + ($customer['30']|default:0) + ($customer['60']|default:0) + ($customer['90']|default:0))|number_format:2}
                                </td>
                            </tr>
                        {/foreach}
